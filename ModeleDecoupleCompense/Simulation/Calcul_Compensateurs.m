...
Ga_angle_phi = avPhase(pole_des_angle, FTplaque(1,1), (18*pi/180), 0);
Ga_angle_theta = avPhase(pole_des_angle, FTplaque(2,2), (20*pi/180), 0); 
FTplaque_phi_comp = series(FTplaque(1,1),1.8*Ga_angle_phi);
FTplaque_theta_comp = series(FTplaque(2,2), Ga_angle_theta);
[num_phi_comp, den_phi_comp] = tfdata(FTplaque_phi_comp, 'v');
...
stepinfo(feedback(FTplaque_phi_comp, 1))

%PI
z_PI_angle = real(pole_des_angle(1))/6;
num_PI_angle = [1 -z_PI_angle];
den_PI_angle = [1 0];
Kp_angle = abs(polyval(conv(den_PI_angle, den_phi_comp), pole_des_angle(1))/polyval(conv(num_PI_angle, num_phi_comp), pole_des_angle(1)));
G_PI_angle = tf(1.5*Kp_angle*num_PI_angle, den_PI_angle);

%Cascader le PI à la FT compensée avec AvPh
FTplaque_phi_comp2 = series(FTplaque_phi_comp, G_PI_angle);
FTplaque_theta_comp2 = series(FTplaque_theta_comp, G_PI_angle);
[num_Phi_Vphi_comp2, den_Phi_Vphi_comp2] = tfdata(FTplaque_phi_comp2, 'v');
[num_Theta_Vtheta_comp2, den_Theta_Vtheta_comp2] = tfdata(FTplaque_theta_comp2, 'v');

%Vérification des performances
errRPech_angle = den_Phi_Vphi_comp2(end)/num_Phi_Vphi_comp2(end);
disp('Performances FTplaque des angles compensée AvPh et PI:')
stepinfo(feedback(FTplaque_phi_comp2, 1))
