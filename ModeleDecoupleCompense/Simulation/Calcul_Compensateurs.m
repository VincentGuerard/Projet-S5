...
Ga_angle_phi = avPhase(pole_des_angle, FTplaque(1,1), 0, 1);
Ga_angle_theta = avPhase(pole_des_angle, FTplaque(2,2), 0, 1); 
...
numGa_z = [Ka_z*alpha_z*T_z Ka_z*alpha_z];
denGa_z = [alpha_z*T_z 1];
Ga_z = tf(numGa_z, denGa_z);

FTplaque_AvPh_z = series(Ga_z, FTplaque(3,3));

figure
hold on
rlocus(FTplaque_AvPh_z)
rlocus(FTplaque(3,3))
